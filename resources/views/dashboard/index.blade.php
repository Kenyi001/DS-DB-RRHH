...
@section('page-description', 'Resumen ejecutivo del sistema RRHH - YPFB')

@section('content')
<div x-data="dashboardData()" x-init="loadData()" class="space-y-6">
    
    <!-- Debug temporal (remover en producción) -->
    @if(isset($stats['contratos']['debug']) && config('app.debug'))
    <div class="bg-red-50 border border-red-200 rounded-lg p-4">
        ...
        <div class="mt-2 text-sm text-red-700 grid grid-cols-2 gap-4">
            <div>
                ...
                <p>• Total vigentes: {{ $stats['contratos']['vigentes'] }}</p>
                <p>• Por vencer: {{ $stats['contratos']['porVencer'] }}</p>
            </div>
            <div>
                <p><strong>Análisis real:</strong></p>
                <p>• Indefinidos (sin fecha): {{ $stats['contratos']['debug']['indefinidos'] ?? 0 }}</p>
                <p>• Con fecha de fin: {{ $stats['contratos']['debug']['con_fecha_fin'] ?? 0 }}</p>
                <p>• Fechas en pasado: {{ $stats['contratos']['debug']['fechas_pasado'] ?? 0 }}</p>
                <p>• Fechas futuras: {{ $stats['contratos']['debug']['fechas_futuro'] ?? 0 }}</p>
            </div>
        </div>
        @if(($stats['contratos']['debug']['fechas_incorrectas'] ?? 0) > 0)
        <div class="mt-2 p-2 bg-red-100 rounded text-xs">
            <strong>🚨 PROBLEMA CRÍTICO:</strong> 
            {{ $stats['contratos']['debug']['fechas_incorrectas'] }} contratos tienen fechas anteriores a 2020 (datos incorrectos)
            
            @if(isset($stats['contratos']['debug']['ejemplos_fechas_malas']) && $stats['contratos']['debug']['ejemplos_fechas_malas']->count() > 0)
                <div class="mt-1">
                    <strong>Ejemplos:</strong>
                    @foreach($stats['contratos']['debug']['ejemplos_fechas_malas'] as $ejemplo)
                        {{ $ejemplo->NumeroContrato }}: {{ $ejemplo->FechaFin }};
                    @endforeach
                </div>
            @endif
