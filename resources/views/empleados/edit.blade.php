...
@section('title', 'Editar Empleado - ' . $empleado->Nombres . ' ' . $empleado->ApellidoPaterno)
...
<div x-data="empleadoForm()" class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
                ...
                <ol role="list" class="flex items-center space-x-4">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('empleados.show', $empleado->IDEmpleado) }}" class="ml-4 text-sm font-medium text-gray-500 hover:text-gray-700">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <span class="ml-4 text-sm font-medium text-gray-700">Editar</span>
                        </div>
                    </li>
                </ol>
            </nav>
            ...
            <p class="mt-1 text-sm text-gray-600">
                Modifica la información de {{ $empleado->Nombres }} {{ $empleado->ApellidoPaterno }}
            </p>
        </div>
    ...
    <!-- Formulario -->
    <form method="POST" action="{{ route('empleados.update', $empleado->IDEmpleado) }}" class="space-y-8">
        @csrf
        @method('PUT')
                    ...
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <input type="date" 
                                   ...
                                   id="FechaNacimiento" 
                                   value="{{ old('FechaNacimiento', $empleado->FechaNacimiento ? \Carbon\Carbon::parse($empleado->FechaNacimiento)->format('Y-m-d') : '') }}"
                                   required
                    ...
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6">
                            ...
                            <textarea name="Direccion" 
                                      id="Direccion" 
                                      rows="3"
                                      class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('Direccion') border-red-300 @enderror">{{ old('Direccion', $empleado->Direccion) }}</textarea>
                            @error('Direccion')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-6 gap-6">
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <select name="IDDepartamento" 
                                ...
                                @foreach($departamentos as $departamento)
                                    <option value="{{ $departamento->IDDepartamento }}" {{ old('IDDepartamento', $empleado->IDDepartamento) == $departamento->IDDepartamento ? 'selected' : '' }}>
                        ...
                        <div class="col-span-6 sm:col-span-3">
                            ...
                            <input type="date" 
                                   name="FechaIngreso" 
                                   id="FechaIngreso" 
                                   value="{{ old('FechaIngreso', $empleado->FechaIngreso ? \Carbon\Carbon::parse($empleado->FechaIngreso)->format('Y-m-d') : '') }}"
                                   required
                                        ...
                                        <div class="mt-2 text-sm text-red-700">
                                            <p>
                                                @if($empleado->FechaBaja)
                                                    Fecha de baja: {{ \Carbon\Carbon::parse($empleado->FechaBaja)->format('d/m/Y') }}
                                                @endif
                                            </p>
                                            @if($empleado->MotivoBaja)
                                                <p>Motivo: {{ $empleado->MotivoBaja }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
