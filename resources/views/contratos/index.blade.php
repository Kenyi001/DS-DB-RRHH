...
<div x-data="contratosData()" x-init="loadData()" class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="p-6">
            <form method="GET" action="{{ route('contratos.index') }}" class="space-y-4 lg:space-y-0 lg:grid lg:grid-cols-4 lg:gap-4">
                        ...
                        @foreach($contratos as $contrato)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @if($contrato->FechaFin && $contrato->FechaFin <= now()->addDays(30) && $contrato->Estado)
                                        <div class="mt-1">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        @if($contrato->Estado)
                                            <button @click="confirmDelete({{ $contrato->IDContrato }}, '{{ $contrato->empleado->Nombres }} {{ $contrato->empleado->ApellidoPaterno }}')" 
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Mostrando
                            <span class="font-medium">{{ $contratos->firstItem() }}</span>
                            a
                            <span class="font-medium">{{ $contratos->lastItem() }}</span>
                            de
                            <span class="font-medium">{{ $contratos->total() }}</span>
                            contratos
                        </p>
                    </div>
                    <div>
        ...
        @else
            ...
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">
                    @if(request()->hasAny(['search', 'estado', 'empleado_id']))
